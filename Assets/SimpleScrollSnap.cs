...
namespace DanielLochner.Assets.SimpleScrollSnap {
    ...
    public  class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler {
        public static SimpleScrollSnap instance { get; private set; }
        #region Fields
        ...
        public string SaveNearestPanel;
        private bool useSavedNearestPanel = true;
        ...
        public void SelectPanel() {
            if (useSavedNearestPanel) {
                useSavedNearestPanel = false;
            }
            else {
                nearestPanel = GetNearestPanel();
            }
            foreach(ButtonIndex buttons in FindObjectsOfType<ButtonIndex>()) {
                if (buttons.LevelIndex == nearestPanel) {
            ...
            AdjustScaleOfPanels(nearestPanel);
        ...
        public void SaveAndLoadNearestPanel() {
            int pervous = PlayerPrefs.GetInt(SaveNearestPanel, nearestPanel);
            if (pervous != nearestPanel) {
                PlayerPrefs.SetInt(SaveNearestPanel, nearestPanel);
                PlayerPrefs.Save();
                print("SAVE PANELLL :" + nearestPanel);
            }
        }

        public void LoadAndLoadNearstPanel() {
            nearestPanel = PlayerPrefs.GetInt(SaveNearestPanel, nearestPanel);
            print("LOad PANELLL :" + nearestPanel);
        }
        private void OnApplicationQuit() {
            SaveAndLoadNearestPanel();
        }
        // تابعی برای تغییر مقیاس پنل‌ها
        public void AdjustScaleOfPanels(int selectedPanel) {
            ButtonSessionManager buttonSessionManager = FindObjectOfType<ButtonSessionManager>();
            StartCoroutine(buttonSessionManager.MoveChildrenBack());
            // تمام پنل‌ها را پیدا کنید
            foreach (var panel in Panels) {
                // اگر پنل انتخاب شده باشد، مقیاس را افزایش دهید
                if (panel == Panels[selectedPanel]) {
                    panel.transform.localScale = Vector3.one * 1f; // تغییر مقیاس پنل انتخاب شده
                }
                else {
                    panel.transform.localScale = Vector3.one * 0.5f; // کاهش مقیاس پنل‌های دیگر
                }
            }
        }
